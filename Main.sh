...
while getopts ":fc:hse:n:ib:r:" opt; do
case $opt in
...
elif [[ $Isfresh -eq 0 && $Isinc -eq 0 ]]
then
echo "Specifiy type of instalaation as frsh with -f or incremental with -i"
exit 1

elif [[ $Isfresh -eq 1 && $Isinc -eq 1 ]]
then
echo "Cannot combine -f & -i modes!!exiting"
exit 1
...
then
        ...
        if [[ $Isco -eq 0 && $Isstan -eq 0 ]]
        then
                echo "Specify proper arguments like -ic 'Ips' or -isn 'Ips' -e 'Ips'"
                exit 1
        fi
        ...
        echo "NMSIPS: $Nips and EMSIPS: ${!emsMap[@]} "
                if [[ $nmshot -eq 1 ]]
                then
                finc 'NhotStan' "$Nips"             #Passing emsMap[] by default
                else
                finc 'Stan' "$Nips"                   #Passing emsMap[] by default
                fi
        fi
fi
